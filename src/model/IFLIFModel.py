...
from layer.IFLayer import IF_Layer
...
class IFLIFModel:
    def __init__(self, preSize, preMin, preMax, postSize, logger=None):
        """
        Two-layer SNN model with a simple IF layer on the presynaptic side, and a LIF layer on the
        postsynaptic side.
        ...
        self.optimizer = None
        self.logger = logger
        self.W = np.random.rand(self.postSize, self.preSize)

        # IF reteg def egy dimenziora
        self.pre = IF_Layer(preSize)

        # LIF_K reteg def
        self.post = LIF_K_Layer(self.postSize, preNeurons=self.preSize)
    ...
    def resetWeightsGauss(self):
        ...
        :return:
        """
        self.W = np.random.normal(0.5, 0.05, (self.postSize, self.preSize))
